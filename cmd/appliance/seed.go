...
func seedRun(cmd *cobra.Command, args []string, opts *seedOpts) error {
	...
	exportRequest := a.APIClient.AppliancesApi.AppliancesIdExportPost(ctx, appliance.GetId())
	...
	seed, response, err := exportRequest.SSHConfig(*sshConfig).Authorization(token).Execute()
	if err != nil {
	...
	outputfile := filepath.Join(path, fmt.Sprintf("%s_seed.json", appliance.GetId()))
	if err := os.WriteFile(outputfile, encodedSeed, 0644); err != nil {
		return err
	}
